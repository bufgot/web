...
import (
	...
	hertzAdapter "github.com/bufgot/web/engine/hertz"
)

func main() {
	// Create Hertz adapter and router via unified interface
...
func helloHandler(ctx web.Context) error {
	return ctx.JSON(http.StatusOK, map[string]string{"message": "Hello from Hertz!"})
}
...
func startServer(router web.Router) error {
	...
	go func() {
		println("Hertz server starting on :8080")
