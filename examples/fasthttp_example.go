...
import (
	...
	fasthttpAdapter "github.com/bufgot/web/engine/fasthttp"
)

func main() {
	// Create FastHTTP adapter and router via unified interface
...
func helloHandler(ctx web.Context) error {
	return ctx.JSON(http.StatusOK, map[string]string{"message": "Hello from FastHTTP!"})
}
...
func startServer(router web.Router) error {
	...
	go func() {
		println("FastHTTP server starting on :8080")
