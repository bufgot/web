...
func main() {
	// Create Chi adapter and router via unified interface
...
func helloHandler(ctx web.Context) error {
	return ctx.JSON(http.StatusOK, map[string]string{"message": "Hello from Chi!"})
}
...
func startServer(router web.Router) error {
	...
	go func() {
		println("Chi server starting on :8080")
