...
import (
	...
	fiberAdapter "github.com/bufgot/web/engine/fiber"
)

func main() {
	// Create Fiber adapter and router via unified interface
...
func helloHandler(ctx web.Context) error {
	return ctx.JSON(http.StatusOK, map[string]string{"message": "Hello from Fiber!"})
}
...
func startServer(router web.Router) error {
	...
	go func() {
		println("Fiber server starting on :8080")
